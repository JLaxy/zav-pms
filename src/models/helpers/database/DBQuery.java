...
public class DBQuery {
    ...
    public DrinkVariant getDrinkVariantBySize(int productsNameId, String size) {
        ...
        try (Connection con = this.zavPMSDB.createConnection();
            ...
            stmt.setInt(1, productsNameId);
            String dbFormattedSize = StringHelper.convertSizeToDatabaseFormat(size);
            stmt.setString(2, dbFormattedSize);

            System.out.println("Executing query: SELECT * FROM drink_product WHERE products_name_id = " + productsNameId
                    + " AND TRIM(size) = '" + dbFormattedSize + "' AND isVoided = 0");
            ...
            if (result.next()) {
                ...
                System.out.println("Drink variant not found for products_name_id = " + productsNameId + " and size = '"
                        + size + "'");
            }
            result.close();
        } catch (Exception e) {
    ...
    public ObservableList<OrderProduct> getOrderProducts(User loggedInUser) {
        ...
        try (Connection con = this.zavPMSDB.createConnection();
            ...
            while (result.next()) {
                ...
                orderProducts
                        .add(new OrderProduct(productName, size, quantity, amount, discountedPrice, stockSufficient));
            }
        } catch (Exception e) {
            PopupDialog.showErrorDialog(e, this.getClass().getName());
        }
