...
import enums.DiscountCardTypes;
...
import enums.PreferredUnits;
 ...
 public class DBQuery {
     ...
     public ObservableList<FoodVariant> getFoodProducts(String product) {
        ...
        query = "SELECT food_product.id, products_name_id, regular_price, serving_size_id, available_count, discounted_price, isVoided, products_name.product_name FROM `zav-pms-db`.food_product JOIN products_name ON products_name_id = products_name.id WHERE isVoided = 0 AND products_name.product_name LIKE ?";
     ...
     public Map<String, Object> getStockExpenses(int stockID) {
     ...
     public ObservableList<String> getDrinkSizes(int productId) {
         ...
         String query = "SELECT DISTINCT size, preferred_unit_id FROM drink_product WHERE products_name_id = ? AND isVoided = 0";
         try (Connection con = this.zavPMSDB.createConnection();
                 PreparedStatement stmt = con.prepareStatement(query)) {
             ...
             while (result.next()) {
                 ...
                 int unitId = result.getInt("preferred_unit_id");
                 String formattedSize;
                 if (unitId == PreferredUnits.Units.LITERS.getValue()) { // Assuming 3 indicates Liters
                     formattedSize = String.format("%.2fL", size / 1000); // Convert ml to L and format to 2 decimal
                                                                          // places
                 } else {
                     formattedSize = String.format("%.0fml", size); // Format ml as integer
                 }
     ...
     public ObservableList<String> getFoodSizes(int productId) {
         ObservableList<String> sizes = FXCollections.observableArrayList();
         String query = "SELECT DISTINCT product_serving_size.size FROM food_product " +
                 "JOIN product_serving_size ON food_product.serving_size_id = product_serving_size.id " +
                 "WHERE products_name_id = ? AND isVoided = 0";
         try (Connection con = this.zavPMSDB.createConnection();
                 PreparedStatement stmt = con.prepareStatement(query)) {
             stmt.setInt(1, productId);
             ResultSet result = stmt.executeQuery();
             System.out.println("Executing query: " + query); // Debugging statement
             System.out.println("With productId: " + productId); // Debugging statement
             while (result.next()) {
                 String size = result.getString("size");
                 System.out.println("Found size: " + size); // Debugging statement
                 sizes.add(size); // Ensure this matches the column name in your database
             }
             result.close();
             System.out.println("Sizes fetched from database: " + sizes); // Debugging statement
         } catch (Exception e) {
             PopupDialog.showErrorDialog(e, this.getClass().getName());
             System.out.println("Error executing query: " + e.getMessage()); // Debugging statement
         }
         return sizes;
     }
 
     public StockProductType.Type getProductType(int productId) {
         String query = "SELECT stock_product_type_id FROM products_name WHERE id = ?";
         try (Connection con = this.zavPMSDB.createConnection();
                 PreparedStatement stmt = con.prepareStatement(query)) {
             stmt.setInt(1, productId);
             ResultSet result = stmt.executeQuery();
             if (result.next()) {
                 int typeId = result.getInt("stock_product_type_id");
                 for (StockProductType.Type type : StockProductType.Type.values()) {
         ...
         try (Connection con = this.zavPMSDB.createConnection();
                 PreparedStatement stmt = con.prepareStatement(
                         "SELECT * FROM food_product " +
                                 "JOIN product_serving_size ON food_product.serving_size_id = product_serving_size.id " +
                                 "WHERE products_name_id = ? AND product_serving_size.size = ? AND isVoided = 0")) {
             stmt.setInt(1, productsNameId);
     ...
     public void printAvailableSizes(int productsNameId) {
         try (Connection con = this.zavPMSDB.createConnection();
              PreparedStatement stmt = con.prepareStatement(
                      "SELECT size FROM drink_product WHERE products_name_id = ?")) {
             stmt.setInt(1, productsNameId);
             ResultSet result = stmt.executeQuery();
             System.out.println("Available sizes for product ID " + productsNameId + ":");
             while (result.next()) {
     ...
     public DrinkVariant getDrinkVariantBySize(int productsNameId, String size) {
         DrinkVariant retrievedDrinkVariant = null;
         try (Connection con = this.zavPMSDB.createConnection();
              PreparedStatement stmt = con.prepareStatement(
                      "SELECT * FROM drink_product WHERE products_name_id = ? AND TRIM(size) = ? AND isVoided = 0")) {
             stmt.setInt(1, productsNameId);
             String dbFormattedSize = convertSizeToDatabaseFormat(size);
             stmt.setString(2, dbFormattedSize);
     
             System.out.println("Executing query: SELECT * FROM drink_product WHERE products_name_id = " + productsNameId + " AND TRIM(size) = '" + dbFormattedSize + "' AND isVoided = 0");
     
             ResultSet result = stmt.executeQuery();
             if (result.next()) {
                 System.out.println("Drink variant found in database: " + result.getInt("id"));
                 retrievedDrinkVariant = new DrinkVariant(
                         result.getInt("id"),
                         "",  // Placeholder for product_name if not part of this table
                         result.getInt("products_name_id"),
                         result.getDouble("size"),
                         result.getDouble("price"),
                         result.getInt("available_count"),
                         result.getInt("critical_level"),
                         result.getBoolean("isVoided"),
                         result.getDouble("discounted_price"),
                         result.getInt("preferred_unit_id")
                 );
             } else {
                 System.out.println("Drink variant not found for products_name_id = " + productsNameId + " and size = '" + size + "'");
             }
             result.close();
         } catch (Exception e) {
             PopupDialog.showErrorDialog(e, this.getClass().getName());
         }
         return retrievedDrinkVariant;
     }
     
     public String convertSizeToDatabaseFormat(String size) {
         if (size.endsWith("L")) {
     ...
     public ObservableList<Stock> getStockRequirements(FoodVariant foodVariant) {
         ...
         try (Connection con = this.zavPMSDB.createConnection();
              PreparedStatement stmt = con.prepareStatement(
                      "SELECT stock.id, stock_name, stock.quantity, unit_measure_id, stock_type_id, critical_level, stock.isVoided FROM stock_required JOIN stock ON stock_required.stock_id = stock.id WHERE food_product_id = ?")) {
             stmt.setInt(1, foodVariant.getId());
             ResultSet result = stmt.executeQuery();
             while (result.next()) {
                 Stock stock = new Stock(result.getInt("id"), result.getString("stock_name"),
                         result.getDouble("quantity"),
                         result.getInt("unit_measure_id"), result.getInt("stock_type_id"),
                         result.getInt("critical_level"), result.getBoolean("isVoided"));
                 stockRequirements.add(stock);
             }
             result.close();
         } catch (Exception e) {
             PopupDialog.showErrorDialog(e, this.getClass().getName());
     ...
     public ObservableList<OrderProduct> getOrderProducts(User loggedInUser) {
         ObservableList<OrderProduct> orderProducts = FXCollections.observableArrayList();
         String query = "SELECT product_name, size, quantity, amount, discounted_price FROM orders WHERE user_id = ?";
         try (Connection con = this.zavPMSDB.createConnection();
              PreparedStatement stmt = con.prepareStatement(query)) {
             stmt.setInt(1, loggedInUser.getId()); // Assuming you have the logged-in user's ID
             ResultSet result = stmt.executeQuery();
             while (result.next()) {
                 ...
                 double discountedPrice = result.getDouble("discounted_price");
     
                 // Determine stock sufficiency
                 boolean stockSufficient = checkStockSufficiency(productName, size, quantity);
     
                 orderProducts.add(new OrderProduct(productName, size, quantity, amount, discountedPrice, stockSufficient));
             }
         } catch (Exception e) {
             PopupDialog.showErrorDialog(e, this.getClass().getName());
         }
     ...
     private boolean checkStockSufficiency(String productName, String size, int quantity) {
         try (Connection con = this.zavPMSDB.createConnection()) {
             ...
             if (productType == StockProductType.Type.BEVERAGE) {
                 ...
                 if (drinkVariant != null && drinkVariant.getAvailable_count() >= quantity) {
                     return true;
                 }
             } else if (productType == StockProductType.Type.FOOD) {
                 // Get the food variant
                 FoodVariant foodVariant = getFoodVariantBySize(productId, size);
                 if (foodVariant != null) {
                     ObservableList<Stock> stockRequirements = getStockRequirements(foodVariant);
                     for (Stock stock : stockRequirements) {
     ...
     public Map<String, Object> getBeverageExpenses(int beverageID) {
         ...
         try (Connection con = this.zavPMSDB.createConnection();
                 PreparedStatement stmt = con.prepareStatement(
                         "SELECT * FROM zav-pms-db.stock_product_expenses WHERE stock_product_type_id = 1 AND stock_id = ?;")) {
     ...
     public ObservableList<DiscountCard> getDiscountCards() {
        ObservableList<DiscountCard> discountCards = FXCollections.observableArrayList();
        String query = "SELECT senior_pwd_ids.id_number, senior_pwd_ids.fname, senior_pwd_ids.mname, senior_pwd_ids.lname, senior_pwd_ids.suffix, senior_pwd_id_type.type, senior_pwd_id_type.id as type_id FROM senior_pwd_ids JOIN senior_pwd_id_type ON senior_pwd_ids.id_type = senior_pwd_id_type.id";
        try (Connection con = this.zavPMSDB.createConnection();
             PreparedStatement stmt = con.prepareStatement(query);
             ResultSet result = stmt.executeQuery()) {
            while (result.next()) {
                discountCards.add(new DiscountCard(
                    result.getString("id_number"),
                    result.getString("fname"),
                    result.getString("mname"),
                    result.getString("lname"),
                    result.getString("suffix"),
                    result.getString("type"),  // Use String here
    ...
    public boolean isDiscountCardExists(String idNumber) {
        String query = "SELECT COUNT(*) FROM senior_pwd_ids WHERE id_number = ?";
        try (Connection con = this.zavPMSDB.createConnection();
             PreparedStatement stmt = con.prepareStatement(query)) {
            ...
            e.printStackTrace();  // Use appropriate logging or error handling
        }
        return false;
    }
 
     public boolean saveDiscountCard(SeniorPwdId card) {
         String query = "INSERT INTO senior_pwd_ids (id_number, fname, mname, lname, suffix, id_type) VALUES (?, ?, ?, ?, ?, ?)";
         try (Connection con = this.zavPMSDB.createConnection();
              PreparedStatement stmt = con.prepareStatement(query)) {
             stmt.setString(1, card.getIdNumber());
             stmt.setString(2, card.getFname());
             stmt.setString(3, card.getMname());
             stmt.setString(4, card.getLname());
             stmt.setString(5, card.getSuffix());
             stmt.setInt(6, card.getIdType());
             stmt.executeUpdate();
             return true;
         } catch (Exception e) {
             PopupDialog.showErrorDialog(e, this.getClass().getName());
     ...
     public ObservableList<DiscountCardType> getDiscountCardTypes() {
         ObservableList<DiscountCardType> cardTypes = FXCollections.observableArrayList();
         String query = "SELECT * FROM senior_pwd_id_type";
         try (Connection con = this.zavPMSDB.createConnection();
              PreparedStatement stmt = con.prepareStatement(query);
              ResultSet result = stmt.executeQuery()) {
             while (result.next()) {
             ...
             System.out.println("Fetched Discount Card Types from Database: " + cardTypes);
         } catch (Exception e) {
             PopupDialog.showErrorDialog(e, this.getClass().getName());
         }
         return cardTypes;
