...
public class ReportManager {
        ...
        private final String REPORTS_FOLDER_PATH = "generatedreports/";

        public ReportManager(User loggedInUser) {
                this.model = new ReportModel();
                this.loggedInUser = loggedInUser;
        }

        public String createDailyPeriodicSales(ReportTimePeriods.TimePeriod timePeriod, String date) {
                String fileName = REPORTS_FOLDER_PATH + timePeriod.getPeriodString() + "PeriodicReport_"
                ...
                try (PdfWriter writer = new PdfWriter(fileName);
                        ...
                        transactionsTable
                                        .addCell(new Cell().add(
                                                        new Paragraph(String.valueOf(periodicSalesStats.get("income")))
                                                                        .setTextAlignment(TextAlignment.CENTER)));
                        transactionsTable.addCell(new Cell().add(new Paragraph("Expenses"))
                                        .setTextAlignment(TextAlignment.RIGHT));
                        transactionsTable
                                        .addCell(new Cell().add(new Paragraph(
                                                        String.valueOf(periodicSalesStats.get("expenses")))
                                                        .setTextAlignment(TextAlignment.CENTER)));
        ...
        private Paragraph getEndParagraph() {
                return new Paragraph("----- END OF REPORT -----").setBold().setFontSize(10)
                                .setTextAlignment(TextAlignment.CENTER).setMarginTop(20);
        }

        private Paragraph getFooter() {
                return new Paragraph("Report Generated by \"" + this.loggedInUser.getUname() + "\" at "
                                + Security.getSystemName() + "; " + DateHelper.getCurrentDateTimeString())
                                .setTextAlignment(TextAlignment.RIGHT).setFontSize(8).setMarginTop(100);
        }
}
