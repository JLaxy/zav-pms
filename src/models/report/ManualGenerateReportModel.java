...
public class ManualGenerateReportModel {
    ...
    public boolean generaterReorderQueueReport(ReportTimePeriods.TimePeriod timePeriod, String date,
            User loggedInUser) {
        String fileName = new ReportManager(loggedInUser).createReorderQueue(timePeriod, date);
        try {
            if (fileName != null) {
                // Logging Action to Database
                this.controller.getDBManager().query.logAction(loggedInUser.getId(), loggedInUser.getUname(),
                        UserLogActions.Actions.GENERATED_REORDER_QUEUE_REPORT.getValue(),
                        DateHelper.dateTimeToString(DateHelper.getCurrentDateTime()), "");
                if (PopupDialog
                        .confirmOperationDialog(
                                "Report Generated! Do you want a copy to be sent to your email?") != JOptionPane.YES_OPTION)
                    return true;
                if (Emailer.sendReport(loggedInUser.getEmail(), timePeriod.getPeriodString() + " Reorder Queue Report",
                        loggedInUser, fileName)) {
